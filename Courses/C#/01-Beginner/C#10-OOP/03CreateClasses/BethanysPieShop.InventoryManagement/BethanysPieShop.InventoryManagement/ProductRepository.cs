...
namespace BethanysPieShop.InventoryManagement
{
    ...
    // Repository should be between the data, and the code that uses the data. 
    //Code that sits between the database & the app
    {
        ...
        public List<Product> LoadProductsFromFile()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < productsAsString.Length; i++)
                {
                    ...
                    //Product product = new RegularProduct(productId, name, description, new Price() { ItemPrice = itemPrice, Currency = currency }, unitType, maxItemsInStock);
